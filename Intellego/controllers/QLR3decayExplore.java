...
/**
* Another version of the Q-learner whose explore rate decays over time
*
* @author Graham Ritchie
*/
public class QLR3decayExplore extends Controller
{
	/******************************************************************
	...
	*/
	public void begin()
	{
		...
		while (running)
		{
			...
			// choose action randomly according to explore rate
			if(rand(exploreRate)==0)
			{
				action=rand(ACTIONS-1);
				
				// increment explore rate (this means we will explore less)
				exploreRate++;
			}
			else // and according to Q table the rest
			{
				action=Q(state);
			}
